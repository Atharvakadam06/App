...
function NavItem({ path, icon: Icon, label, isActive, badge }) {
  return (
    ...
    >
      ...
      <span className="hidden xl:block text-[14px] font-semibold truncate">{label}</span>
    </NavLink>
  );
}
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex fixed left-0 top-0 h-full w-[72px] xl:w-[248px] bg-white dark:bg-[#080b14] flex-col z-50 border-r border-slate-100 dark:border-[#0e1322] transition-all duration-300 safe-area-top">
        {/* Logo */}
        <div className="px-3 xl:px-4 pt-6 pb-5 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden xl:block text-xl font-black text-slate-900 dark:text-white tracking-tight">StuGrow</span>
          </div>
        </div>

        {/* Nav */}
        <nav className="flex-1 px-2 xl:px-3 py-1 space-y-1 overflow-y-auto no-scrollbar">
        ...
        {/* Bottom */}
        <div className="p-2 xl:p-3 border-t border-slate-100 dark:border-[#0e1322] space-y-1 shrink-0 safe-area-bottom">
          <NavLink
            to="/profile"
          ...
          >
            ...
            <span className="hidden xl:block text-sm font-semibold">Settings</span>
          </NavLink>
          ...
          >
            ...
            <span className="hidden xl:block text-sm font-semibold">Log out</span>
          </button>
        </div>
      </aside>
      ...
      {!hideMobileNav && (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[9999] safe-area-bottom">
          <div className="mx-3 mb-2 rounded-2xl bg-white/95 dark:bg-[#080b14]/95 backdrop-blur-2xl border border-slate-200/60 dark:border-white/[0.06] shadow-xl shadow-slate-900/10 dark:shadow-black/40">
            <div className="flex items-center justify-around py-1.5 px-1">
              ...
              >
                <div className={`p-2.5 rounded-xl transition-all duration-200 ${
                  ...
                  <LayoutGrid className={`w-6 h-6 transition-transform duration-200 ${isMoreActive ? 'scale-105' : ''}`} />
                </div>
              </button>
